...
export class JiraApiClient {
  ...
  /**
   * Send comment to a Jira issue
   * POST to https://jirastage.deltaww.com/rest/api/2/issue/{issuekey}/comment
   */
  async sendComment(issueKey: string, commentBody: string): Promise<ApiResponse> {
    try {
      // In test mode, simulate a successful response
      if (this.testMode) {
        console.log('[TEST MODE] Simulating Jira comment API call');
        console.log('[TEST MODE] Issue Key:', issueKey);
        console.log('[TEST MODE] Comment:', commentBody);
        ...
        return {
          success: true,
          data: {
            message: 'Test mode: Comment would be sent to Jira',
            issue_key: issueKey,
            timestamp: new Date().toISOString(),
          },
          status_code: 201,
      ...
      // Construct the comment payload according to Jira API format
      const payload = {
        body: commentBody,
      };

      // Make actual API call to Jira
      // POST to /rest/api/2/issue/{issueKey}/comment
      const endpoint = `/rest/api/2/issue/${issueKey}/comment`;
      console.log(`📤 Sending comment to Jira: ${endpoint}`);

      const response: AxiosResponse = await this.client.post(endpoint, payload);

      console.log(`✅ Comment sent successfully to issue ${issueKey}`);
      ...
      console.error(`Failed to send comment to Jira issue ${issueKey}:`, error);
